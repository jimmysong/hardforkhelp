...
from tx import Tx, TxIn, TxOut, BCDTx, BTGTx, SBTCTx, BCHTx, B2XTx, BTVTx
...
class TxTest(TestCase):
    ...
    def test_btv_sign(self):
        wif = 'cUio9ok9CXDfByfGEiRi8snZmwSaAbnvdbmfg72wegPdkX4b7Cq5'
        prev_tx = 'c9338d3cb5ce17fcfaee8ccadd7513110aee3148fbce1bc978d245d180fd7ad8'
        prev_index = 1
        prev_value = 1000000000
        destination = 'msDmp5xD5hNLsiyasWjNpuxPuz1BfY2G9R'
        amount = 996000000

        priv_key = PrivateKey.parse(wif)
        prev_script_pubkey = BTVTx.get_address_data(priv_key.point.address())['script_pubkey'].serialize()
        tx_in = TxIn(unhexlify(prev_tx), prev_index, b'', 0xffffffff, b'\x00', prev_value, prev_script_pubkey)
        script = BTVTx.get_address_data(destination)['script_pubkey'].serialize()
        tx_out = TxOut(amount, script)
        tx = BTVTx(2, [tx_in], [tx_out], 0, testnet=True)
        tx.sign(priv_key)
        want = ""
        self.assertTrue(tx.verify())
        self.assertEqual(tx.serialize().hex(), want)
